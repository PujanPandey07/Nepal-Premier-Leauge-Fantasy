...
export default function MockPayment() {
  ...
  const amountNPR = rawAmount ? Number(rawAmount) / 100 : 0

  const [paying, setPaying] = useState(false)
  const [failed, setFailed] = useState(false)
  const [selectedMethod, setSelectedMethod] = useState(0)

  // Dynamically pull base API URL from axiosInstance defaults
  const apiBaseUrl = axiosInstance.defaults.baseURL || ''

  const handlePay = (status = 'Completed') => {
    if (!pidx) {
      setFailed(true)
      return
    }
    ...
    setPaying(true)
    const verifyEndpoint = `/api/payments/verify/?pidx=${encodeURIComponent(pidx)}&status=${encodeURIComponent(status)}`
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-100 p-4">
        <div className="w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-lg">
          <p className="mb-1 font-semibold text-red-600">Invalid Payment Parameters</p>
          <p className="mb-5 text-xs text-gray-500">
            Missing or invalid transaction credentials (pidx/amount).
          </p>
          <button
      ...
      <div className="w-full max-w-sm overflow-hidden rounded-2xl bg-white shadow-lg">
        ...
        <div className="flex items-center gap-3 bg-purple-600 px-6 py-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm">
            <span className="text-lg font-black text-purple-600">K</span>
          </div>
          <div>
            <p className="text-lg font-bold leading-tight text-white">Khalti</p>
            <p className="text-xs text-purple-200">Digital Wallet & Payment</p>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-5 rounded-xl border border-gray-100 bg-gray-50 p-4">
            <p className="mb-0.5 text-xs text-gray-500">Paying to</p>
            <p className="font-semibold text-gray-800">NPL Fantasy Cricket</p>
            <p className="mt-0.5 text-xs text-gray-400">Wallet Top Up</p>
          </div>
          ...
          <div className="mb-6 space-y-2">
            <p className="mb-2 text-xs font-medium text-gray-500">Pay via</p>
              ...
              >
                <div
                  className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                    selectedMethod === i ? 'border-purple-500' : 'border-gray-300'
                  }`}
                >
                  {selectedMethod === i && (
