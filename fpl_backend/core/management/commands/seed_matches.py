...
class Command(BaseCommand):
    help = "Seeds or resets NPL S2 match schedule starting tomorrow at 4:00 PM local time."

    @transaction.atomic
    def handle(self, *args, **options):
        tournament, _ = Tournament.objects.get_or_create(
            name="Nepal Premier League S2",
            defaults={"season": "2025"}
        )
        ...
        # Calculates tomorrow at 4:00 PM (16:00) local time
        local_now = timezone.localtime(timezone.now())
        replay_start = (local_now + timedelta(days=1)).replace(
        ...
        for i, m in enumerate(MATCHES, start=1):
            ...
            home_team = Cricket_Team.objects.get(
                name=TEAM_NAME_MAP[m["team1"]])
            away_team = Cricket_Team.objects.get(
                name=TEAM_NAME_MAP[m["team2"]])
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully processed {len(MATCHES)} matches! "
                f"First match set for: {replay_start.strftime('%Y-%m-%d %I:%M %p %Z')}. "
